...
def calc_r2(data, model, device):
    with torch.no_grad():
        output = model.to("cpu")(data.tensors[0][:,0:2].to("cpu")).cpu().detach().numpy()
    model.to(device)
...
if __name__ == "__main__":
    ...
    idx_residual = np.linspace(0, len(coordinates) - 1, len(coordinates), dtype=int)

    # These are the points used for training! NOT the same as the reference solution!
    x = np.hstack((coordinates, idx_residual[:, np.newaxis]))
    ...
    writerAdam = tb.SummaryWriter(log_dir=tbPath + options["output"] + "Adam", comment="Adam_training")
